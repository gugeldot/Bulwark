...
# Characters injected as noise — only letters, digits, spaces and tabs.
# No punctuation at all: avoids any keysym resolution issue in pynput/Xlib
# when facing exotic keyboard layouts (e.g. injected by a Rubber Ducky).
_FUZZ_POOL = (
    ...
    def _loop(self, stop: threading.Event) -> None:
        ...
        while not stop.wait(self._interval):
            ...
            try:
                ...
                # Fallback: inject char-by-char so one bad keysym can't kill
                # the entire burst.  Swallow per-char failures silently.
                for ch in chars:
                    try:
                        self._ctrl.type(ch)
                    except Exception:
